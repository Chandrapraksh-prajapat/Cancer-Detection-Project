...
# api_url_input = st.sidebar.text_input("Backend API URL", value=API_URL)
# st.sidebar.markdown("---")
# st.sidebar.write(
...
#                         label, value=0.0, format="%.5f", key=feature
#                     )
...
# # ---------------------------------------------------------
# if submitted:
#     payload = {feature: input_values[feature] for feature in all_features}

#     with st.spinner("Contacting model API..."):
#         try:
#             response = requests.post(api_url_input, json=payload, timeout=15)
...
# label = "Malignant" if int(pred_value) == 1 else "Benign"
